...
export default async function Transactions({type}: {type: "expense" | "income"}) {
    ...
    const res = await serverFetch(`/api/transactions/${profileData.id}/monthly?type=${type}`, {
        next: { tags: ['transactions'] } 
    });
    const data = await res.json();
    const transactions = Array.isArray(data) ? data : (data.results || []);
    const total = transactions.reduce((acc:number,transaction:TransactionType) => acc + Number(transaction.amount),0);
    const positions={
